...
def compare_reports(old_dir: Path, new_dir: Path) -> int:
    ...
    if old_json_text != new_json_text:
        ...
        print(_unified_diff(old_json_text, new_json_text, "old/security-report.json", "new/security-report.json"))

    if old_md != new_md:
        failed = True
        print("ERROR: Markdown reports differ after timestamp normalisation.")
        print(_unified_diff(old_md, new_md, "old/security-report.md", "new/security-report.md"))
    ...
    print(
        "Parity passed: JSON and Markdown match for "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compare old and extracted auditor report directories."
    )
