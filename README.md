...
- opt-in capability comparisons against an explicit previous audit report
...
Compare current capability summaries against an explicitly selected previous audit JSON:

```bash
uv run decky-audit \
  --all \
  --plugins-file config/plugins.txt \
  --baseline-report accepted-security-report.json \
  --output-dir security-reports
```

`--baseline-report` accepts either one plugin report or an aggregate `security-report.json`. Aggregate baselines are matched to current reports by normalized GitHub repository identity. The auditor does **not** decide which report is accepted, trusted, or previously distributed; the consuming store or review system owns that decision and supplies the baseline explicitly. A missing repository match, a pre-capability report, or an unsupported capability schema produces an explicit `comparison unavailable` result instead of inferring previous behavior.

Capability comparisons distinguish status transitions such as `not_observed → observed`, `observed → not_observed`, and coverage transitions involving `unknown`. In particular, `unknown → observed` is reported as `now_observed_after_unknown`, not as proof that the behavior was newly introduced. The comparison also tracks reviewer-relevant evidence-profile changes including network destinations, native-binary identities, source/release-difference categories, and capability rule profiles. Changed current evidence reuses the redacted, immutable provenance generated by the reviewer evidence layer.

Comparison output is additive and opt-in. When a baseline is supplied, each matched report may include `reviewer_capability_comparison_schema_version` and `reviewer_capability_comparison`; without `--baseline-report`, existing JSON and Markdown report shapes are unchanged.
...
- capability comparison semantics once a baseline is explicitly supplied
...
- selection, persistence, and lifecycle of accepted comparison baselines
- allowlist decisions and approvals
- schedules and audit cadence
- cache and report retention policy
- review workflow integration

Consumer-owned configuration is supplied explicitly at runtime. The standalone repository keeps only generic examples under [`examples/consumer`](examples/consumer); it does not maintain a store catalog, accepted-release database, or scheduled full-store audit.

## Roadmap

1. Persist authoritative accepted-release baselines in consuming review systems and feed them into capability comparison.
2. Expose a reusable GitHub Action and official Decky review integrations.
...
The auditor performs static inspection only. It treats release artifacts, source repositories, and supplied baseline reports as untrusted input and does not import or execute plugin code. Findings, reviewer capability summaries, and comparisons are evidence, not proof that a plugin is malicious or safe.
